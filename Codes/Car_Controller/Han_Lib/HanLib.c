...
void han_Timer_Encoder(TIM_TypeDef* TIMx, u16 LineNumber, u16 CatchAngle, FunctionalState IfEnableIT)
{
	TIM_TimeBaseInitTypeDef TIM_TimeBaseStructure;
...
void han_Timer_Counter(TIM_TypeDef* TIMx, u16 FreqHz, FunctionalState IfEnableIT)
{
	TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
	RCC_ClocksTypeDef RCC_Clocks;
	uint32_t Periph_Clock = 0;
	...
	TIM_TimeBaseStructure.TIM_Prescaler = Periph_Clock - 1; 				//定时器预分频 APBx /(TIM_Prescaler + 1) = 1M
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;				//向上计数模式
	TIM_TimeBaseStructure.TIM_Period = (1000000.0 / (double)FreqHz) - 1;			//自动重装值 (1M / FreqHz + 1) = 1k
	TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1; 				//设置时钟分割: TDTS = Tck_tim
	TIM_TimeBaseInit(TIMx, &TIM_TimeBaseStructure);
